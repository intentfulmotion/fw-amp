...
void PatternRenderer::process() {
  if (_pattern.compare("theater-chase-rainbow") == 0)
    ...
    theaterChase(_lights->randomColor(), 10, 50);
...
void PatternRenderer::allColor(Color color) {
...
void PatternRenderer::allRandom() {
  for (auto channel : channels) {
    _lights->colorLEDs(channel.first, 0, channel.second.leds, _lights->randomColor());
  }
  _lights->render();
}
...
void PatternRenderer::theaterChase(Color c, int cycles, int speed) { // TODO direction
  ...
  for(int i = 0; i < cycles; i++){
    for(int j = 0; j < simultaneous; j++){
      for (auto channel : channels) {
        ...
        (*controller)[idx] = _lights->randomColor();
      }
    }
    _lights->render();
    delay(speed);
