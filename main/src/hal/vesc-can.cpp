...
VescCan::VescCan() {
  ...
  transmitQueue = xQueueCreate(50, sizeof(std::string*));
  receiveQueue = xQueueCreate(50, sizeof(std::string*));
...
void VescCan::startup() {
  ...
  // temporary
  PowerStatus temp;
  temp.batteryPresent = false;
  temp.charging = false;
  temp.doneCharging = true;
  temp.percentage = 100;
  temp.level = PowerLevel::Charged;
  powerStatus = temp;
...
void VescCan::process() {
  ...
  if (initialized) {
    ...
    if (millis() - status.balance.timestamp > interval)
      requestBalanceData();
  } 
...
void VescCan::parseBalanceData(bool isProxyRequest) {
  ...
  status.balance.balanceState = (BalanceState) readInt16FromBuffer(24 + offset, isProxyRequest);
  status.balance.switchState = (SwitchState) readInt16FromBuffer(26 + offset, isProxyRequest);
...
bool VescCan::detectDirection() {
  ...
  if ((value >= _directionThreshold && axisValue % 2 == 0) || value <= _directionThreshold)
...
int16_t VescCan::readInt16FromBuffer(int start, bool isProxyRequest) {
  int16_t intVal = (
    ((int16_t) (isProxyRequest ? proxybuffer : buffer).at(start) << 8) +
...
int32_t VescCan::readInt32FromBuffer(int start, bool isProxyRequest) {
  int32_t intVal = (
    ((int32_t) (isProxyRequest ? proxybuffer : buffer).at(start) << 24) +
...
void VescCan::proxyOut(uint8_t *data, int size, uint8_t crc1, uint8_t crc2) {
  ...
  xQueueSend(transmitQueue, &pStr, 0);
}
#endif